...
async function getStockOut(id: string) {
...
export default async function StockOutDetailPage({ params }: { params: { id: string } }) {
  const item = await getStockOut(params.id);
  if (!item) notFound();

  const rows = [
    { label: "สินค้า", value: [item.products_mf?.name, item.products_mf?.model].filter(Boolean).join(" · ") },
    { label: "จำนวน", value: `${item.quantity} ${item.products_mf?.unit ?? ""}` },
    { label: "วันที่ขาย", value: formatDate(item.sold_date) },
    { label: "ลูกค้า", value: item.customer_name },
    ...
    <div className="p-4 max-w-lg mx-auto w-full space-y-5">
      <div className="pt-2">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-xl font-bold text-gray-900">รายละเอียดการขาย</h1>
            <p className="text-sm text-gray-500 mt-0.5">{formatDate(item.sold_date)}</p>
          </div>
          <Link
            href={`/stock-out/${item.id}/edit`}
