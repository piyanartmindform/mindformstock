          ...
          {open.length > 0 && (
            <div className="space-y-2">
              {open.map((item: any) => {
                ...
                return (
                  <Link key={item.id} href={`/stock-out/new?expected=${item.id}`}>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-400 mt-1">
                            {item.customer_name}{item.project_name ? ` · ${item.project_name}` : ""} · เหลืออีก {remaining} {item.products_mf?.unit}
                          </p>
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">
                          {item.customer_name}{item.project_name ? ` · ${item.project_name}` : ""} · ส่งแล้ว {item.sold_quantity}/{item.expected_quantity} {item.products_mf?.unit}
